...
/* ── Data ── */
const AMIGOS_ONLINE = [
  { id: 1, nombre: 'AlexNeural', carrera: 'Ing. de Inteligencia Artificial', avatar: null, initials: 'AN', color: '#FF5B2E', status: 'online', statusText: 'En curso: IA Avanzado' },
  { id: 2, nombre: 'Luna_Core', carrera: 'Artist · Battleborn', avatar: null, initials: 'LC', color: '#C4107A', status: 'online', statusText: 'Artist · Battleborn' },
];
const AMIGOS_OFFLINE = [
  { id: 3, nombre: 'Chipancini-Banini', carrera: 'Offline', avatar: null, initials: 'CB', color: '#6B7280', status: 'offline', statusText: 'Offline' },
];
const CHATS = [
  { id: 1, nombre: 'Maya Sterling', mensaje: 'escribiendo...', hora: 'AHORA', avatar: null, initials: 'MS', color: '#FF5B2E', escribiendo: true },
  { id: 2, nombre: 'Marcus Chen', mensaje: '¿Revisaste el último módulo de IA?', hora: '14:21', avatar: null, initials: 'MC', color: '#C4107A', escribiendo: false },
  { id: 3, nombre: 'Elena Rodríguez', mensaje: 'Enviado un archivo .py', hora: 'AYER', avatar: null, initials: 'ER', color: '#A855F7', escribiendo: false },
];
const MENSAJES_MAYA = [
  { id: 1, tipo: 'recibido', texto: '¡Hola! ¿Lograste optimizar el núcleo del sistema ALBERT para la simulación de mañana?', hora: '14:10' },
  { id: 2, tipo: 'enviado', texto: 'Sí, acabo de terminar. Reduje la latencia de respuesta en un 15% usando el nuevo protocolo orbital.', hora: '14:11' },
  { id: 3, tipo: 'recibido', texto: '¡Increíble! ¿Puedes enviarme los registros de depuración? Me gustaría compararlos con mi estructura.', hora: '14:12' },
...
const FriendsIcon = ({ active, isDark }) => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none"
    stroke={active ? (isDark ? '#FF5B2E' : '#FF8430') : (isDark ? 'rgba(255,255,255,0.50)' : 'rgba(0,0,0,0.40)')}
...
const Avatar = ({ item, size = 36 }) => (
  <div style={{
    ...
    fontSize: size * 0.3, fontWeight: 700, color: item.color,
...
/* ── SocialPopup ── */
const SocialPopup = ({ isDark, onClose }) => {
    ...
    <>
      {selectedUser ? (
        ...
        <div style={s.userCard}>
          <button style={s.cardClose} onClick={() => setSelectedUser(null)}><CloseIcon isDark={isDark} /></button>
          ...
          <div style={s.userCardStatus}>
            ...
            <span style={{ fontSize: 11, color: isDark ? 'rgba(255,255,255,0.40)' : 'rgba(0,0,0,0.40)', marginLeft: 6 }}>ING SISTEMAS</span>
          </div>
          <button style={s.sendMsgBtn} onClick={() => { setOpenChat(selectedUser); setTab('chat'); setSelectedUser(null); }}>
      ...
      {openChat ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
          <div style={s.chatHeader}>
            <button style={s.backBtn} onClick={() => setOpenChat(null)}><BackIcon isDark={isDark} /></button>
          ...
          <div style={s.inputRow}>
            <input style={s.chatInput} placeholder="Escribe un mensaje encriptado..." value={msgInput} onChange={e => setMsgInput(e.target.value)} />
        ...
        <>
          <div style={s.chatListHeader}>
            ...
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={isDark ? 'rgba(255,255,255,0.50)' : 'rgba(0,0,0,0.40)'} strokeWidth="2" strokeLinecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          </div>
          <div style={s.searchWrap}>
            <SearchIcon isDark={isDark} />
  ...
  const renderAdd = () => (
    <>
      ...
      {ADD_FRIENDS.map(u => (
        <div key={u.id} style={s.addFriendRow} onClick={() => setSelectedUser({ ...u, status: 'online', statusText: 'Online' })}>
          <Avatar item={u} />
          <div style={s.friendName}>{u.nombre}</div>
        </div>
      ))}
  ...
  return (
    <div style={s.popup}>
      {/* TABS */}
      <div style={s.tabs}>
        <button style={{ ...s.tabBtn, ...(tab === 'friends' ? s.tabActive : {}) }} onClick={() => { setTab('friends'); setSelectedUser(null); }}>
          <FriendsIcon active={tab === 'friends'} isDark={isDark} />
        </button>
        <button style={{ ...s.tabBtn, ...(tab === 'chat' ? s.tabActive : {}) }} onClick={() => { setTab('chat'); setSelectedUser(null); }}>
          <ChatIcon active={tab === 'chat'} isDark={isDark} />
        </button>
        <button style={{ ...s.tabBtn, ...(tab === 'add' ? s.tabActive : {}) }} onClick={() => { setTab('add'); setSelectedUser(null); }}>
...
const getStyles = (isDark) => ({
  ...
  userCard: {
    ...
    border: `1px solid ${isDark ? 'rgba(255,255,255,0.10)' : 'rgba(255,132,48,0.20)'}`,
    borderRadius: 16,
    padding: '20px 16px',
    display: 'flex',
    flexDirection: 'column',
