...
const Calendar = ({ theme = 'light', onToggleTheme }) => {
  ...
  const handleDrop = (e, fecha, hora = null) => {
    ...
    if (dragging) {
      setEvents(prev => prev.map(ev =>
        ev.id === dragging.id ? { ...ev, fecha, ...(hora ? { hora } : {}) } : ev
      ));
    }
    setDragging(null);
    setDragOver(null);
  ...
  const IconBell = () => (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
      <path d="M9 2a5.5 5.5 0 0 1 5.5 5.5c0 2.5.5 4 1.5 5H2c1-1 1.5-2.5 1.5-5A5.5 5.5 0 0 1 9 2z" stroke={isDark ? '#FFFFFF' : 'rgba(0,0,0,0.70)'} strokeWidth="1.4"/>
      <path d="M7 16a2 2 0 0 0 4 0" stroke={isDark ? '#FFFFFF' : 'rgba(0,0,0,0.70)'} strokeWidth="1.4" strokeLinecap="round"/>
    </svg>
  );
  const IconSocial = () => (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      <circle cx="5" cy="4" r="2.5" stroke="#fff" strokeWidth="1.3"/>
      <circle cx="9.5" cy="5" r="2" stroke="#fff" strokeWidth="1.3"/>
      <path d="M1 12c0-2.2 1.8-4 4-4s4 1.8 4 4" stroke="#fff" strokeWidth="1.3"/>
      <path d="M9.5 9c1.4 0 2.5 1.1 2.5 2.5" stroke="#fff" strokeWidth="1.3"/>
    </svg>
  );
      ...
      <div style={s.main}>
        ...
        <div style={s.topbar}>
          ...
          <div style={s.topbarRight}>
            ...
            <button style={s.iconBtn}><IconBell /></button>
            <button style={s.socialBtn}><IconSocial /> Social</button>
          </div>
        </div>
                  ...
                  <div style={s.weekViewHeader}>
                    ...
                    {getWeekDays().map((d, i) => (
                      <div key={i} style={s.weekViewHeaderCell}>
                        ...
                        <span style={{ ...s.weekDayNum, ...(d.getDate() === today.getDate() ? s.weekDayNumToday : {}) }}>
                          {d.getDate()}
                        </span>
                      </div>
                    ))}
                          ...
                          >
                            <div style={{ borderLeft: `3px solid ${ev.color}`, paddingLeft: 8 }}>
                              ...
                              <div style={s.kanbanCardMeta}>
                                <IconClock />
                                <span style={{ marginLeft: 4 }}>{ev.fecha} · {ev.hora}</span>
                              </div>
                              <div style={{ ...s.kanbanCardMeta, marginTop: 4 }}>
                                <IconTag />
                                <span style={{ marginLeft: 4, color: ev.color }}>{ev.materia}</span>
                              </div>
                            </div>
                          </div>
                        ))}
            ...
            <div style={s.modalHeader}>
              <div style={{ borderLeft: `4px solid ${eventDetail.color}`, paddingLeft: 12 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}>
                  <span style={{ ...s.modalBadge, background: eventDetail.color + '22', color: eventDetail.color }}>
                    {eventDetail.materia}
                  </span>
            ...
            <div style={s.modalBody}>
              <div style={s.modalRow}>
                ...
                <div>
                  ...
                  <div style={s.modalRowVal}>{eventDetail.fecha} · {eventDetail.hora}</div>
                </div>
              </div>
              <div style={s.modalRow}>
                ...
                <div>
                  ...
                  <div style={{ ...s.modalRowVal, color: eventDetail.color }}>{eventDetail.materia}</div>
                </div>
              </div>
              <div style={s.modalRow}>
                ...
                <div>
                  ...
                  <div style={s.modalRowVal}>{eventDetail.estado.charAt(0).toUpperCase() + eventDetail.estado.slice(1)}</div>
                </div>
              </div>
            </div>
...
const getStyles = (isDark) => ({
  ...
  calMain: {
    ...
    border: `1px solid ${isDark ? 'rgba(255,255,255,0.06)' : 'rgba(220,193,181,0.30)'}`,
    borderRadius: 16,
    overflow: 'hidden',
    minWidth: 0,
  },
  ...
  weekViewHeaderCell: {
    ...
    alignItems: 'center',
    gap: 2,
    borderLeft: `1px solid ${isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.05)'}`,
  },
  weekDayName: {
    fontSize: 10,
    fontWeight: 600,
  ...
  modalRowVal: {
    fontSize: 13,
    fontWeight: 500,
    color: isDark ? '#FFFFFF' : 'rgba(0,0,0,0.80)',
  },
  modalFooter: {
    display: 'flex',
