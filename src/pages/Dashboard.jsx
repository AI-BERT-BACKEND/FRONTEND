...
const Dashboard = () => {
  ...
  const [alertas] = useState(ALERTAS_INIT);
  ...
  const alertCardBorder = alertNivel === 'rojo'
    ? `1px solid ${pulseBorder ? 'rgba(240,7,7,0.60)' : 'rgba(240,7,7,0.18)'}`
    : alertNivel === 'amarillo'
  ...
  return (
    <AppLayout>
      ...
      <section
        style={{ ...s.card, border: alertCardBorder, transition: 'border-color 0.35s ease' }}
        aria-label="Estado académico"
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <img
            src={
              alertNivel === 'rojo' ? AibertRojo
              : alertNivel === 'amarillo' ? AibertAmarillo
              : AibertVerde
            }
            alt="AI.BERT"
