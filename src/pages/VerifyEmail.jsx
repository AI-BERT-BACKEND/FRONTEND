...
const VerifyEmail = ({ theme = 'light', onToggleTheme }) => {
  ...
  const handleChange = (val, idx) => {
    ...
    const n = [...code]; n[idx] = val; setCode(n); setError('');
    if (val && idx < 5) inputs.current[idx + 1]?.focus();
  };

  const handleKeyDown = (e, idx) => { if (e.key === 'Backspace' && !code[idx] && idx > 0) inputs.current[idx - 1]?.focus(); };

  const handlePaste = (e) => {
    e.preventDefault();
    const p = e.clipboardData.getData('text').replace(/\D/g, '').slice(0, 6);
    const n = [...code]; p.split('').forEach((ch, i) => { n[i] = ch; }); setCode(n);
  ...
  const handleResend = () => { if (!canResend) return; setTimer(60); setCanResend(false); setCode(['','','','','','']); setError(''); };
  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.card}>
        <div style={s.logoWrap}><img src={Logo} alt="AI.BERT logo" style={s.logo} /></div>
        <h1 style={s.title}>{isDark ? 'Verifica tu cuenta' : 'VERIFICA TU CORREO'}</h1>
        <p style={s.description}>Ingresa el código de verificación que<br />enviamos a tu correo institucional.</p>
          ...
          <div style={s.codeRow}>
            {code.map((digit, idx) => (
              <input key={idx} ref={el => inputs.current[idx] = el}
                style={{ ...s.codeInput, ...(error ? s.codeInputError : {}), ...(digit ? s.codeInputFilled : {}) }}
        ...
        <button style={s.btn} onClick={handleSubmit}>Verificar correo</button>

        <div style={s.resendWrap}>
          <span style={s.resendLabel}>Reenviar código en</span>
          {!canResend && <span style={s.timer}>{formatTime(timer)}</span>}
          <button style={{ ...s.resendBtn, opacity: canResend ? 1 : 0.4, cursor: canResend ? 'pointer' : 'default' }}
...
const getStyles = (isDark) => ({
  ...
  card: {
    ...
    background: isDark ? '#171717' : '#FEFAF9',
    border: `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(220,193,181,0.30)'}`,
    borderRadius: 20, padding: '36px 40px 32px', width: '100%', maxWidth: 400,
    boxShadow: isDark
  ...
  logoWrap: { width: 64, height: 64, marginBottom: 18, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  logo: { width: '100%', height: '100%', objectFit: 'contain' },
  title: {
    fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: isDark ? 22 : 20, fontWeight: 800,
    background: isDark ? 'linear-gradient(90deg, #FF5B2E, #C4107A)' : 'linear-gradient(90deg, #FF8430, #F7306D)',
    WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
    marginBottom: 10, letterSpacing: isDark ? 0 : '0.04em',
  },
  description: { fontSize: 13, color: isDark ? 'rgba(255,255,255,0.60)' : 'rgba(0,0,0,0.55)', lineHeight: 1.65, marginBottom: 24 },
  field: { width: '100%', marginBottom: 20, textAlign: 'left' },
  label: { display: 'block', fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: isDark ? 'rgba(255,255,255,0.60)' : 'rgba(0,0,0,0.55)', marginBottom: 10 },
  ...
  codeInput: {
    ...
    color: isDark ? '#FFFFFF' : 'rgba(0,0,0,0.85)', fontSize: 18, fontWeight: 600,
    fontFamily: "'Plus Jakarta Sans', sans-serif", textAlign: 'center', outline: 'none',
    transition: 'border-color 0.2s, box-shadow 0.2s', boxSizing: 'border-box',
  },
  codeInputFilled: { borderColor: isDark ? 'rgba(196,16,122,0.60)' : 'rgba(255,132,48,0.60)', boxShadow: `0 0 0 2px ${isDark ? 'rgba(196,16,122,0.15)' : 'rgba(255,132,48,0.12)'}` },
  codeInputError: { borderColor: '#F00707', boxShadow: '0 0 0 2px rgba(240,7,7,0.12)' },
  errorRow: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, fontSize: 11.5, color: '#F00707', marginTop: 8, fontWeight: 500 },
  btn: {
  ...
  resendLabel: { fontSize: 12, color: isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.45)' },
  timer: { fontSize: 13, fontWeight: 600, color: isDark ? 'rgba(255,255,255,0.70)' : 'rgba(0,0,0,0.65)', fontFamily: "'Plus Jakarta Sans', sans-serif" },
  resendBtn: { fontSize: 12, color: isDark ? '#FF5B2E' : '#F7306D', fontWeight: 600, background: 'none', border: 'none', padding: 0, fontFamily: "'Poppins', sans-serif", cursor: 'pointer' },
});
